...
public class RedisManagerImpl implements RedisManagerService {
    ...
    public boolean expireKey(String key, long timeout) {
        return redisTemplate.expire(key, timeout, TimeUnit.SECONDS); // 设置过期时间
    }
    ...
    public boolean hasKey(String key) {
        return redisTemplate.hasKey(key); // 检查指定键是否存在
    }

    @Override
    public boolean persist(String key) throws Exception {
        // 移除过期时间
        return redisTemplate.persist(key);// 返回布尔值，表示是否成功移除过期时间
    }
