...
public interface FriendApplyMapper {
    ...
    // 根据 ID 删除好友申请
    int deleteFriendApply(Integer id);

    // 更新好友申请状态
    int updateFriendApply(FriendApplyDo friendApply);

    // 根据 ID 查询好友申请
    FriendApplyDo getFriendApplyById(Integer id);

    // 根据申请用户 ID 查询所有被申请的用户list
    List<FriendApplyDo> getMyAppliesBySenderId(Integer senderId);

    // 根据处理用户 ID 查询所有对他发出申请的用户list
    List<FriendApplyDo> getHandleAppliesByHandlerId(Integer handleId);

    // 根据申请用户 ID 和处理用户 ID 联合查询
    FriendApplyDo getFriendApplyByUserIds(@Param("applyUserId") Integer applyUserId, @Param("handleUserId") Integer handleUserId);
    ...
    List<NewUserItemBo> getHandleMyAddUserResponseList(Integer applyUserId);

    // 根据handleUserId 查询 List<NewUserItemBo>
    List<NewUserItemBo> getAddMeRequestList(Integer handleUserId);
}
