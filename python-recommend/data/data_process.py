...
def negative_sample(data: List[tuple], ratio = 1,threshold = 0):
    ...
    :param ratio:           负正样本比例(ratio = 1全负样本)
    :param threshold:       权重阈值，权重大于或者等于此值为正样例，小于此值既不是正样例也不是负样例(threshold = 0表示都是正样本不可推荐)
    :return:                带上负样本的数据集
    """
    print("开始负采样(未曾接触的样本)")
    global negative_sample_weight
    ...
    print('positive_set长度:',len(user_positive_set))
    print('unpositive_set长度:',len(user_unpositive_set))
    ...
    with ProcessPoolExecutor(max_workers=os.cpu_count()//2, initializer=negative_sample_init, initargs=(ratio, negative_sample_weight)) as executor:
        sampled_negative_items = executor.map(negative_sample_single_user, item_for_user_positive_set, item_for_user_unpositive_set, chunksize=100)
...
# 初始化
def negative_sample_init(_ratio,_negative_sample_weight):
    global item_set, ratio, negative_sample_weight
    negative_sample_weight = _negative_sample_weight
    ratio = _ratio
    item_set = set(_negative_sample_weight.keys())
...
def negative_sample_single_user(positive_set: set, unpositive_set: set):
    ...
    # 权重存储
    weights = np.array([negative_sample_weight[item_id] for item_id in valid_negative_list], dtype=float)
    # 归一化
    weights /= weights.sum()  # 负样本采集权重
    # 概率采样
    sample_choice = np.random.choice(range(len(valid_negative_list)), n_negative_sample, False, weights)
...
def neaten_id(data: List[tuple])\
    ...
    for user_id_old, item_id_old, label in data:
        ...
        if item_id_old not in item_id_new:
            item_id_new[item_id_old] = n_item
            n_item += 1
        new_data.append((user_id_new[user_id_old], item_id_new[item_id_old], label))
    return new_data, n_user, n_item, user_id_new, item_id_new


def split(data: List[tuple], test_ratio=0.4, shuffle=True, ensure_positive=False) \
...
def prepare_topK(train_data: List[Tuple[int, int, int]],
    ...
    # 从data获取用户的物品集合
    def get_user_item_set(data: List[Tuple[int, int, int]], only_positive=False):# 是否只考虑标签为1的数据
