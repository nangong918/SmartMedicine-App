package com.czy.baseutil.reflect;
...
public class ReflectImpl {
    ...
    private final Map<String, Class<?>> reflectMethodMap;

    public ReflectImpl(ReflectTestInterface reflectTestInterface) {
        this.reflectTestInterface = reflectTestInterface;
        reflectMethodMap = new HashMap<>();
        generateReflectMap();
    }
    ...
    private boolean isApiMethod(Method method) {
        ...
        return !methodName.equals("getClass") &&
                ...
                !methodName.equals("clone"); // 其他需要过滤的方法
    }

    private void generateReflectMap(){
        Method[] methods = reflectTestInterface.getClass().getMethods();
        for (Method method : methods) {
            if(isApiMethod(method)){
                ...
                // 获取返回泛型类型
                Class<?> genericReturnType = parameterTypes[parameterTypes.length - 1];

                // Ld
                String liveDataName = method.getName() + "Ld";
                reflectMethodMap.put(liveDataName, genericReturnType);
            }
        }
    }
